...
class VllmConfig(PreTrainedConfig):
    ...
    This policy encodes LeRobot/LIBERO observations into that request and decodes the
    response into LIBERO's 7-D action space.
    """
    ...
    # --- action decoding ---
    # For "libero_7d": the server returns per-key {x,y,z,roll,pitch,yaw,gripper}; we just
    # concatenate them (in LIBERO_ACTION_KEYS order) into the 7-D LIBERO action.
    action_keys: list[str] = field(default_factory=lambda: list(LIBERO_ACTION_KEYS))
    # Must match the LIBERO env control_mode ("relative" delta or "absolute").
    control_mode: str = "relative"
    action_scale: float = 1.0
    gripper_action_open: float = -1.0
    gripper_action_close: float = 1.0
    # libero_7d: the vllm-omni server returns ABSOLUTE actions (current state + predicted
    # delta). Isaac-GR00T's native LIBERO eval feeds the raw DELTA to env.step (relative
    # OSC_POSE), so we subtract the current state to recover it. Default True for this server.
    decode_subtract_state: bool = True
    # Gripper post-processing to match Isaac-GR00T's LIBERO eval
    # (normalize [0,1]->[-1,1], optional sign-binarize, then invert sign).
    libero_gripper_binarize: bool = True
    libero_gripper_invert: bool = True
    # Clip the final action to LIBERO's [-1, 1] box.
